...
import { fmtMoney, sumMoney, carCost, carMargin, carYear, carAcquisitionBase, mhCode, internalCode, carAttention, nowMs, CAR_STATUS, CAR_STATUS_ORDER, NOT_FOR_SALE_STATUSES } from "@/lib/format";
                    ...
                    <tr key={c.id}>
                      ...
                      {flags.seeAcquisition && (
                        <Cell href={href} className="mono text-right">
                          {/* Партнёрские авто — реальная закупка: наценка остаётся
                              внутри группы и себестоимость не увеличивает (правки-2). */}
                          {fmtMoney(carAcquisitionBase(c))}
                        </Cell>
                      )}
                      {flags.seeAcquisition && (
                        <Cell href={href} className="mono text-right text-muted">
