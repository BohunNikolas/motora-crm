...
Next.js 16 + TypeScript + Tailwind v4 + Prisma 6 + Postgres (Neon). Без сторонних UI-библиотек — вся дизайн-система своя в `globals.css`.
...
- **Вся денежная арифметика — только в `src/lib/finance.ts`** (австрийские налоги: Differenzbesteuerung `USt = max(0, продажа − база) × 20%` СВЕРХУ разницы, двухступенчатый расчёт для партнёрских авто, Regelbesteuerung, план/факт). Округление half-up до цента. В компонентах арифметики денег быть НЕ должно. Формулы покрыты `finance.test.ts` (vitest, кейсы §24.1). `format.ts` (carCost/carMargin/dealMargin/carPlannedFinance) — тонкие адаптеры Prisma→finance.
- **Владелец e.U.→OG (§9, переписан в правках-1 и -2):** `currentOwner` = MOTORHOF_OG | MRIYA_MOTORS | A_MOTORS | AUTOHUB (`isPartnerOwner` в `format.ts`). Механизм внутреннего счёта (`plannedInternalTransferPrice`, `internalInvoiceNumber`, `awaitingInternalInvoice`, `ogAcquisitionBasis`, `supplierFinance`) УДАЛЁН — поля остались в схеме как легаси и не используются. Вместо него — **фиктивная наценка** `fictitiousMarkup`: официальный заработок e.U. на внутренней перепродаже, чтобы у налоговой не было вопросов о продажах без прибыли. Заполняется только у партнёрских владельцев (для MOTORHOF_OG зануляется в `ownerDataFromForm`), автоподбор — `suggestedMarkup()` (5% от реальной закупки, мин. € 250, вверх до € 10; константы в `pricing-shared.ts`).
- **Двухступенчатый дифф. НДС (правки-2, `docs/TZ-DUAL-DIFF-VAT.md`):** партнёрское авто на Differenz юридически продаётся дважды, поэтому НДС считается на каждой ступени: `vatEU = (финальная закупочная − стоимость автомобиля) × ставка`, `transferPrice = финальная закупочная + vatEU` (НДС переложен в цену внутреннего счёта — чистый заработок e.U. равен наценке), `vatOG = max(0, продажа − transferPrice) × ставка`. Экономика **сквозная по группе**: маржа одна, наценка в ней НЕ вычитается (`margin = продажа − vatEU − vatOG − реальная закупка − расходы`), себестоимость считается от `costBasis` (у партнёров — реальная закупка). Всё это внутри `computePricing`; в UI и снапшот идут поля `isTwoStage/vatEU/vatOG/transferPrice/realCost`. Для MOTORHOF_OG и для Regelbesteuerung — прежний одноступенчатый расчёт. Ступени, наценка и передаточная цена — под `see.internalPrice` (ADMIN/PARTNER), redaction server-side. Исторические `financialSnapshot` не пересчитываются.
- **Бронь и продажа (§18):** сущность `Sale` (единый цикл RESERVED→COMPLETED→CANCELLED, soft-cancel) — ИСТОЧНИК ИСТИНЫ по продажам. Действия `reserveCar`/`completeSale`/`cancelSale` в actions.ts; бронь конвертируется в продажу (одна запись). RESERVED/SOLD ставятся ТОЛЬКО через эти действия, не `setCarStatus` (guard + `SALE_FLOW_STATUSES`). `completeSale`: обязательные поля §18.2, `buildSaleSnapshot` (замораживает маржу в `financialSnapshot` Json — смена ставки НЕ меняет историю), Mindestpreis-блок+override, освобождение парковки+ParkingMove, автозакрытие открытых задач, §9 awaiting. Правка финансов проданного авто — только admin override (`soldOverride`+причина, `updateCar`). **Deal — воронка лидов (§17), НЕ трогает статус авто** (decoupled в 3e). Дашборд «Продано/Маржа» и любые метрики продаж — по `Sale(COMPLETED, financialSnapshot.finalMargin)`, не по Deal. Data-миграция Deal→Sale: `prisma/migrate-deals-to-sales.mjs` (идемпотентно по `legacyDealId @unique`).
- **Каналы закупки (§11, сокращены в правках-1):** `purchaseChannel` = PRIVAT | AUKTION | HAENDLER (Inzahlungnahme и Import удалены, легаси-значения считаются как PRIVAT); условные поля значимы только для своего канала (`channelDataFromForm` зануляет остальные). Поля канала и цен показывает клиентский `CarPricingBlock`. База приобретения — `carAcquisitionBase`/`costBasis` из `computePricing` (у партнёров реальная закупка, иначе финальная закупочная); база дифф. НДС для Auktion — Fahrzeugpreis, не gesamt. Проверка §11.2: gesamt < Fahrzeugpreis блокируется без override с причиной (только edit.car = ADMIN/PARTNER открывают форму). Import: применимость Differenzbesteuerung НЕ автоматизировать (UNGEKLAERT при нехватке). Закупочные детали на карточке — под `see.acquisition` (redaction от SALES/TECHNICAL).
...
- Себестоимость и маржа — ТОЛЬКО через `carCost`/`carMargin`/`carPricing` (адаптеры к `finance.ts`), нигде не пересчитывать руками: формулы зависят от канала, владельца и налоговой схемы (см. два пункта выше).
