...
- **Каналы закупки (§11):** `purchaseChannel` = PRIVAT | AUKTION | HAENDLER | INZAHLUNGNAHME | IMPORT, условные поля значимы только для своего канала (`channelDataFromForm` зануляет остальные). Клиентский `CarChannelFields` показывает блок нужного канала. `ogAcquisitionBasis` (единая точка базы приобретения OG): партнёр → внутр. счёт (§9) → иначе Auktion → Auktionsrechnung gesamt → иначе Inzahlungnahme → зачётная стоимость → иначе purchasePrice. §24-Einkaufspreis (`ogEinkauf24`): для Auktion по умолчанию = Fahrzeugpreis, НЕ gesamt (§12.2). Проверка §11.2: gesamt < Fahrzeugpreis блокируется без override с причиной (только edit.car = ADMIN/PARTNER открывают форму). Import: применимость Differenzbesteuerung НЕ автоматизировать (UNGEKLAERT при нехватке). Закупочные детали на карточке — под `see.acquisition` (redaction от SALES/TECHNICAL).
